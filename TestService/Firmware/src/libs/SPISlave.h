...
public:
    ...
    bool begin(const uint8_t spi_bus = HSPI) {
        bus_cfg.mosi_io_num = (spi_bus == HSPI) ? MOSI : 13;
        bus_cfg.miso_io_num = (spi_bus == HSPI) ? MISO : 12;
        bus_cfg.sclk_io_num = (spi_bus == HSPI) ? SCK : 14;
        if_cfg.spics_io_num = (spi_bus == HSPI) ? SS : 15;
...
private:
    bool initialize(const uint8_t spi_bus) {
        host = (spi_bus == HSPI) ? HSPI_HOST : HSPI_HOST;
